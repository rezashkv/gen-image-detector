...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["reconstruction", "noise_scale", "stepwise", "grad", "grad_l2"]
    )
...
def main(args):
    ...
    if args.error_type == "noise_scale":
        if args.n_bins > 1:
            ...
            errors = pipe.noise_scale_error_batches(args, train_dataloader, text_embeddings, dft=args.dft)
            for key in errors:
                error = errors[key]
                log_wandb_error_histogram(project="stablediffusion-detection",
        ...
        if args.logger == "wandb":
            for key in errors:
                ...
                log_wandb_error_histogram(project="stablediffusion-detection",
                                          ...
                                          errors=error)
    elif args.error_type == "grad_l2":
        errors = pipe.gradient_l2_error(args, train_dataloader, text_embeddings)
        if args.logger == "wandb":
            for key in errors:
                error = errors[key]
                log_wandb_error_histogram(project="stablediffusion-detection",
