...
from torchvision import transforms, utils
import random
from scripts.helper import OptimizerDetails, get_seg_text
import clip
import os
import inspect
...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    # model = UNet2DConditionModel.from_pretrained(ckpt, subfolder="unet")
...
def get_optimation_details(args):
    ...
    # operation_func = Segmentation(model, Trans)
    operation_func = model
    ...
    operation.watermark_model = model
...
def main():
        ...
        if n in opt.indexes:
            ...
            for multiple_tries in range(opt.trials):
                ...
                samples_ddim, start_zt = sampler.sample(S=opt.ddim_steps,
                                                        ...
                                                        eta=opt.ddim_eta,
                                                        operated_image=torch.ones((1, 1), dtype=torch.float).to(device),
                                                        operation=operation)

                x_samples_ddim = model.module.decode_first_stage(samples_ddim)
                x_samples_ddim_unnorm = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)

                os.makedirs(os.path.join(results_folder, str(int(opt.optim_forward_guidance_wt))), exist_ok=True)
                utils.save_image(x_samples_ddim_unnorm,os.path.join(results_folder, str(int(opt.optim_forward_guidance_wt)),
                                                                    f'new_img_{n}_{multiple_tries}.png'))
    ...
    torch.save(operation.watermark_model.state_dict(), os.path.join(results_folder, "watermark-model.pt"))
