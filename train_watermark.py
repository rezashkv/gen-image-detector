...
from pipelines.watermark_pipeline import StableDiffusionWatermarkPipeline
...
def main(args):
    ...
    pipe = StableDiffusionWatermarkPipeline.from_pretrained(args.model_config_name_or_path)
    scheduler = DDIMScheduler.from_pretrained(args.model_config_name_or_path, subfolder='scheduler',
                                              predict_type=args.prediction_type)
    ...
    for epoch in range(args.num_epochs):
        for step, batch in enumerate(train_dataloader):
            outputs, watermark_outputs = pipe(prompt=batch["caption"], num_inference_steps=args.ddim_num_inference_steps)
            if args.logger == "wandb":
                if not is_wandb_available():
                    raise ImportError("Make sure to install wandb if you want to use it for logging during training.")
                import wandb
                wandb.log({"resnet outputs": watermark_outputs})
