...
from utils import CustomImageFolder, InversableStableDiffusionPipeline, \
...
def get_transforms(aug_str, fname="base", save_images=False):
    ...
    if aug == 'no_aug':
        ...
        transform = transforms.Compose(
            [
                transforms.Resize(256),
                transforms.GaussianBlur((15, 15), (10, 10)),
                transforms.ToTensor(),
            ]
        )
    elif aug == 'combo':
...
def main():
    ...
    parser.add_argument("--wm-method", default="dwtDct", type=str,
        choices=["dwtDct", "dwtDctSvd", "rivaGan", "treeRing", 'watermarkDM', 'stegaStamp'])
    parser.add_argument("--attack", default="diffpure", type=str,
        choices=["diffpure", "diffpure_latent", 'common_augs', 'image_edit'])
    ...
    if args.attack == 'diffpure':
        aug_list = ['no_aug', 'diffpure,0.1', 'diffpure,0.2', 'diffpure,0.3', 'diffpure,0.4']
    ...
    for aug in aug_list:
        ...
        if args.wm_method in ["dwtDct", "dwtDctSvd", "rivaGan"]:
            ...
            from stega_stamp import decode_stega_stamp
    ...
    #### F1 PLOT
    for aug, eval_dict in th_results.items():
        keys = list(eval_dict.keys())
        keys.sort()
        f1 = []
        for key in keys:
            f1.append(2 * eval_dict[key]['tp'] / (2 * eval_dict[key]['tp'] + eval_dict[key]['fp'] + eval_dict[key]['fn']))
        plt.plot(keys, f1, label="{}, max_f1={:.2f}".format(aug, max(f1)))

    plt.axhline(y=2/3, color='r', linestyle='--')
    plt.xlabel('Watermark Threshold')
    plt.ylabel('F1 Score')
    plt.legend()
    plt.savefig(f'results/plots/{args.wm_method}_{args.dataset}_{args.out_fname}_f1.png')
