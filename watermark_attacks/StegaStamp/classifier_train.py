...
class CustomImageFolder(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # save_image(image, f'tmp/{idx}.png')
        # exit()
...
def load_dataset(transform):
    # transform_in = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(), 
    #                                    AddGaussianNoise(0., 0.4)])
    # transform_wm = transforms.Compose([transforms.Resize(224), transforms.ToTensor(),
    #                                    AddGaussianNoise(0., 0.4)])
    # transform_in = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(),])
    # transform_wm = transforms.Compose([transforms.Resize(224), transforms.ToTensor(),])

    # transform = transforms.Compose(transform.transforms +  [AddGaussianNoise(0., 0.4)])
    # norm = preprocess.transforms[-1]

    dataset_in = CustomImageFolder('./images/imagenet/treeRing_no_wm',
                                    transform=transform, y=0)
    dataset_wm = CustomImageFolder('./images/imagenet/treeRing_10k',
                                   transform=transform, y=1, data_cnt = len(dataset_in))

    # dataset_wm = CustomImageFolder('./out',
    #                                transform=transform, y=1)
    # dataset_in = CustomImageFolder('path_to_image_dataset',
    #                                 transform=transform, y=0, data_cnt = len(dataset_wm))
    N = len(dataset_wm)

    print(f'number of watermarked images --> {N}')

    # transform_in = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(), ])
    print(f'number of original images --> {len(dataset_in)}')

    dataset = torch.utils.data.ConcatDataset([dataset_in, dataset_wm])
    print(f'number of images in the dataset --> {len(dataset)}')

    train, test = train_test_split(np.arange(len(dataset_in) + len(dataset_wm)), test_size=0.3)
...
def train_model(model, train_dataloader, val_dataloader, num_epochs=10, lr=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels, _ in tqdm(train_dataloader):
            ...
            # noise_inputs = (inputs + torch.rand_like(inputs) * 2 * epsilon - epsilon).detach()
            # noise_inputs.requires_grad = True
            # outputs = model(noise_inputs).to(device)[:, 0]
            # loss = criterion(outputs, labels)
            # loss.backward()
            # grad = noise_inputs.grad.data
            # adv_inputs = inputs + torch.clamp(noise_inputs + epsilon * grad.sign() - inputs, -epsilon, epsilon)
        ...
        torch.save({'model': model.state_dict()}, './models/classifier_treeRing_norm.pt')
...
def main():
    ...
    train_set, test_set = load_dataset(preprocess)

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=False, num_workers=4)  # check
